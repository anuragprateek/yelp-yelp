...
def main(save_sample, save_categories):
    ...
    sample_rid2cat={}
    sample_size = 70 #len(valid_cats) # This specifies how many cuisines you would like to save 
    ...
    if save_categories:
        ...
        for cat in sample_cat2reviews:
            with open ('categories/' + cat.replace('/', '-').replace(" ", "_") + ".txt" , 'write') as f:
...
def sim_matrix():
    ...
    #vectorizer = TfidfVectorizer(max_df=0.5, max_features=10000,
     #                                min_df=2, stop_words='english',
      #                               use_idf=True)
    vectorizer = CountVectorizer(stop_words='english', max_df=0.5, max_features=10000, min_df=2)
    ...
    cat_sample = []
    all_sample = sorted(random.sample(range(cat_size), cat_size))

    if not os.path.exists('data_catsample50.pickle'):
        sample_size = min(50, cat_size)
        cat_sample = sorted( random.sample(range(cat_size), sample_size) )
        with open('data_catsample50.pickle', 'wb') as f:
            pickle.dump(cat_sample,f)

    else:
        cat_sample = pickle.load( open( "data_catsample50.pickle", "rb" ) )

    print (cat_sample)

    for i, item in enumerate(cat_list):
        if i == all_sample[count]:
            li =  item.split('/')
            cuisine_name = li[-1]
            all_c_names.append(cuisine_name[:-4].replace("_"," "))
            with open ( item ) as f:
                all_text.append(f.read().replace("\n", " "))
    ...
    lda = models.ldamodel.LdaModel(corpus, num_topics=20)
